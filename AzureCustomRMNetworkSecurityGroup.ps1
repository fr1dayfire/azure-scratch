# https://gallery.technet.microsoft.com/Managing-Azure-Network-68bb904a#content
# https://hmmconfused.wordpress.com/2015/01/23/managing-azure-network-security-groups-with-csv-files/
# 
...
# 
# New-AzureRmCustomNetworkSecurityGroup
# Update-AzureRmCustomNetworkSecurityGroup
# Export-AzureRmNetworkSecurityGroup
...
# 
# New-AzureRmCustomNetworkSecurityGroup -CSVPath C:\AzureNetworkSecuriyGroupRules\RulesARM.csv -AzureLocation "North Europe" -ResourceGroupName TEST-RG -NetworkSecurityGroupName NSG-DMZ-1 -Tags @{Name="State";Value="Production"}
...
# Export-AzureRmNetworkSecurityGroup -CSVPath C:\AzureNetworkSecuriyGroupRules\RulesARM.csv -ResourceGroupName TEST-RG -NetworkSecurityGroupName NSG-DMZ-1
...
[CmdletBinding()]
    ...
    if($PortRange.ToString() -match "\*"){
        ...
        if(-not($PortRangeNumbers[0] -in 0..65000)){
            return $false
        }

        if(-not($PortRangeNumbers[1] -in 0..65000)){
            return $false
        }

        return $true
    } elseif($PortRange -in 0..65000){
        #number
        return $true
    } else {
        return $false
